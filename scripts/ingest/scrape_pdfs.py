...
async def can_fetch(session: aiohttp.ClientSession, url: str, config: MainConfig) -> bool:
    ...
    try:
        ...
        async with session.get(robots_txt, headers={'User-Agent': agent}, timeout=10) as resp:
            if resp.status == 200:
...
async def crawl(
    ...
    if mode == 'text':
        saved = await save_text(output_dir, url, soup,
                                rejected_dir=Path(output_dir) / config.rejected_docs_foldername)
        if saved:
            text_files.append(saved)
        all_pdfs.update(pdfs)

    tasks = [crawl(u, session, depth + 1, mode, output_dir, all_pdfs, config)
             for u in links if u not in visited_urls]
...
async def run_scrape(
    ...
    async with aiohttp.ClientSession() as session:
        if mode == 'text':
            texts, _ = await crawl(start_url, session, 0, 'text', output_dir, set(), config)
            results['output_paths'] = list(set(texts))
            if pdf_log:
                with open(pdf_log, 'w', encoding='utf-8') as f:
                    json.dump(list(visited_urls), f, indent=2)
            results.update({'status': 'success', 'message': f'Saved {len(texts)} text files.'})
...
if __name__ == '__main__':
    ...
    config_path = (Path(args.config).resolve() if args.config else
                   Path(os.getenv(ENV_CONFIG_PATH_VAR, '')).resolve() if os.getenv(ENV_CONFIG_PATH_VAR) else
    ...
    out_dir = Path(args.output_dir or config.data_directory or project_root / 'data' / 'scraped_content')
