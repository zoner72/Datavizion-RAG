...
def connect_dynamic_signals(self):
    ...
    if isinstance(checkbox, QCheckBox):
        checkbox.toggled.connect(
            self.handle_embedding_edit_toggle
        )  # Connect to the handler method
        logger.debug(
            "Connected embedding_edit_checkbox signal to handle_embedding_edit_toggle."
        )
    else:
        logger.warning(
            "Embedding edit checkbox widget not found in ui_widgets/settings_widgets."
        )
    ...
    for key in self.rebuild_sensitive_keys:
        ...
        if widget:
            if isinstance(widget, QLineEdit):
                ...
                logger.debug(
                    f"Connected textChanged for '{key}' to mark_rebuild_needed."
                )
                ...
                if hasattr(widget, "valueChanged"):
                    ...
                    logger.debug(
                        f"Connected valueChanged for '{key}' to mark_rebuild_needed."
                    )
                elif hasattr(widget, "currentIndexChanged"):
                    widget.currentIndexChanged.connect(self.mark_rebuild_needed)
                    logger.debug(
                        f"Connected currentIndexChanged for '{key}' to mark_rebuild_needed."
                    )
                else:
                    logger.warning(
                        f"Widget for '{key}' is {type(widget).__name__} but has no valueChanged or currentIndexChanged signal."
                    )
            # Add other widget types that might need rebuild logic if necessary
            else:
                logger.warning(
                    f"Widget for rebuild-sensitive key '{key}' has unhandled type: {type(widget).__name__}"
                )
        else:
            logger.warning(
                f"Widget for rebuild-sensitive key '{key}' not found in settings_widgets."
            )
