...
from PyQt6.QtWidgets import (
    ...
    QWidget, QTextEdit # Import QTextEdit
)
...
def add_config_setting(config_tab, layout, label_text, setting_key, default_value=None,
    ...
    if widget:
        ...
        logger.error(f"Failed to create widget for setting '{setting_key}' (type={widget_type}, default={default_value})")
...
def add_config_setting_with_browse(config_tab, layout, label_text, setting_key, default_value=None, dialog_title="Select File", directory=True):
    ...
    def browse_clicked():
        ...
        if directory:
            path = QFileDialog.getExistingDirectory(config_tab, dialog_title, current_path)
        else:
            path, _ = QFileDialog.getOpenFileName(config_tab, dialog_title, current_path)
