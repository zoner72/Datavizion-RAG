...
from PyQt6.QtCore import QProcess, QProcessEnvironment, Qt, QTimer, pyqtSignal, pyqtSlot
...
try:
    from config_models import MainConfig  # Import necessary models/exceptions

    pydantic_available = True
except ImportError as e:
    logging.critical(
        f"FATAL ERROR: Cannot import Pydantic models in ApiTab: {e}. Tab may fail.",
        exc_info=True,
    )
...
TOOLTIP_TOGGLE_BUTTON_START = (
    "Start the background API server process using settings from Config tab."
)
...
TOOLTIP_URL_LABEL = (
    "URL to access the API when running (based on settings in Config tab)."
)
...
DIALOG_ALREADY_RUNNING = (
    "Server might already be running or the specified port is in use."
)
DIALOG_PROCESS_FAILED_START = "The API server process failed to start. Check console/logs for errors from the server script."
DIALOG_CONFIG_PATH_ERROR = (
    "Could not determine the path to the main configuration file."
)
...
class ApiTab(QWidget):
    ...
    def __init__(
        ...
        if not pydantic_available:
            ...
            layout.addWidget(QLabel("API Tab Disabled: Config system failed."))
        ...
        if not isinstance(config, MainConfig):
            ...
            layout = QVBoxLayout(self)
            layout.addWidget(QLabel("API Tab Disabled: Invalid Configuration."))
            self._disable_init_on_error()
            return
        # Validate and store project_root
        if not isinstance(project_root, Path) or not project_root.is_dir():
            logging.critical(
                f"{log_prefix} Invalid project_root received ({project_root}). Paths will be incorrect."
            )
            # Proceed but log critical error, path finding will likely fail
            QMessageBox.critical(
                self,
                DIALOG_ERROR_TITLE,
                f"Internal Error: Invalid project root path.\n{project_root}",
            )
        ...
        self.stopping_server: bool = (
            False  # Flag to prevent race conditions during stop
        )
        ...
        if not self.config_file_path:
            ...
            QTimer.singleShot(
                ...
                lambda: QMessageBox.warning(
                    self, DIALOG_WARNING_TITLE, DIALOG_CONFIG_PATH_ERROR
                ),
            )
    ...
    def _resolve_config_path(self) -> Optional[Path]:
        ...
        if (
            not hasattr(self, "project_root")
            or not self.project_root
            or not isinstance(self.project_root, Path)
        ):  # Check if attribute exists and is valid
            logger.error(
                "Cannot resolve config path: self.project_root is invalid or not set."
            )
            return None
        try:
            ...
            logger.error(
                f"Error resolving config path relative to project root {getattr(self, 'project_root', 'N/A')}: {e}",
                exc_info=True,
            )
            return None
    ...
    def _load_settings(self):
        ...
        if not pydantic_available or self.config is None:
            logger.warning(
                f"{log_prefix} Cannot load settings: Pydantic or config unavailable."
            )
        ...
        try:
            ...
            api_config = (
                self.config.api
            )  # Assume 'api' key exists due to Pydantic model
            ...
            if isinstance(host_from_config, str) and host_from_config.strip():
                ...
                logger.warning(
                    f"{log_prefix} Invalid API host '{host_from_config}' in config, using default '{DEFAULT_API_HOST}'."
                )
                self.current_host = DEFAULT_API_HOST
            ...
            try:
                ...
                logger.warning(
                    f"{log_prefix} Invalid API port '{port_from_config}' in config, using default {DEFAULT_API_PORT}."
                )
                self.current_port = DEFAULT_API_PORT

            logger.info(
                f"{log_prefix} Settings loaded: Host={self.current_host}, Port={self.current_port}"
            )

        except AttributeError as e:
            logger.error(
                f"{log_prefix} Error accessing config attributes (likely missing 'api' section or fields): {e}. Using defaults."
            )
            self.current_host = DEFAULT_API_HOST
            self.current_port = DEFAULT_API_PORT
        except Exception:
            logger.exception(
                f"{log_prefix} Unexpected error loading settings from config."
            )
            self.current_host = DEFAULT_API_HOST
            self.current_port = DEFAULT_API_PORT
    ...
    def _check_auto_start(self):
        ...
        try:
            ...
            logging.info(
                f"{log_prefix} Auto-start setting from config: {auto_start_enabled}"
            )
            if auto_start_enabled:
                if not self.server_running:
                    logging.info(
                        f"{log_prefix} Auto-start enabled, attempting to start API server..."
                    )
                    self._start_server()  # Call start method directly
                else:
                    logging.info(
                        f"{log_prefix} Auto-start enabled, but server is already running."
                    )
            else:
                logging.info(f"{log_prefix} Auto-start disabled.")
        except AttributeError as e:
            logging.error(
                f"{log_prefix} Error reading auto-start config: {e}. Auto-start skipped."
            )
        except Exception:
    ...
    def update_config(self, new_config: MainConfig):
        ...
        logging.info(
            f"--- APITab: update_config called. New Config ID: {id(new_config)} ---"
        )
        if not pydantic_available:
            return
    ...
    def _start_server(self):
        ...
        if (
            self.server_running
            or self._is_server_process_active()
            or self.stopping_server
        ):
            logging.warning(
                f"{log_prefix} Aborted: Server process active, running={self.server_running}, stopping={self.stopping_server}."
            )
            self._update_ui_state()
            return
        ...
        if not self.config:
            ...
            QMessageBox.critical(
                self, DIALOG_ERROR_TITLE, "Cannot start server: Configuration missing."
            )
            return
        ...
        if not python_exe or not Path(python_exe).is_file():
            logger.critical(
                f"{log_prefix} Aborted: {DIALOG_PYTHON_PATH_ERROR} ('{python_exe}')"
            )
            QMessageBox.critical(
                self, DIALOG_ERROR_TITLE, f"{DIALOG_PYTHON_PATH_ERROR}:\n{python_exe}"
            )
            return
        ...
        try:
            if not self.project_root:
                raise ValueError("Project root not set")  # Add check
            server_script_abs = (
                self.project_root / SERVER_SCRIPT_REL_PATH
            ).resolve()  # Use self.project_root
            if not server_script_abs.is_file():
                raise FileNotFoundError(
                    f"API server script not found: {server_script_abs}"
                )
            ...
            logger.critical(
                f"{log_prefix} Aborted: Error resolving server script path: {e}",
                exc_info=True,
            )
            QMessageBox.critical(
                self, DIALOG_ERROR_TITLE, f"Error finding server script:\n{e}"
            )
            return
        ...
        try:
            ...
            logging.info(
                f"Preparing server start command: {python_exe} {' '.join(command_args)}"
            )
            ...
            logging.debug(
                f"{log_prefix} Setting PYTHONPATH for subprocess: {python_path}"
            )
            logging.debug(
                f"{log_prefix} Setting {ENV_CONFIG_PATH_VAR}={self.config_file_path}"
            )
            ...
            self.server_process.finished.connect(
                lambda code, status: self._handle_process_finished(
                    code, QProcess.ExitStatus(status)
                )
            )
            ...
            logging.exception(
                f"{log_prefix} Unexpected error during QProcess start sequence."
            )
            QMessageBox.critical(
                self,
                DIALOG_ERROR_TITLE,
                f"{DIALOG_SERVER_START_ERROR}\nUnexpected error: {e}",
            )
            self.server_process = None
    ...
    def _stop_server(self):
        ...
        if not self._is_server_process_active():
            logging.info(
                f"{log_prefix} Server process not active. Ensuring state is clean."
            )
            self.server_running = False
            self.stopping_server = False
            if self.server_process is not None:
                ...
                try:
                    self.server_process.disconnect()
                except Exception:
                    pass
                self.server_process = None
            self._update_ui_state()
        ...
        logging.info(
            f"{log_prefix} Attempting to stop server process PID: {self.server_process.processId()}..."
        )
        ...
        try:
            ...
            if self.server_process.waitForFinished(2000):
                ...
                logging.warning(
                    f"{log_prefix} Server did not terminate gracefully after 2s. Sending kill signal..."
                )
                self.server_process.kill()
                if not self.server_process.waitForFinished(1000):
                    logging.error(
                        f"{log_prefix} Server process could not be killed after signal."
                    )
                    QMessageBox.critical(
                        self,
                        DIALOG_ERROR_TITLE,
                        DIALOG_SERVER_STOP_ERROR + "\nProcess may still be running.",
                    )
            ...
            if self.server_process is not None:
                logging.warning(
                    f"{log_prefix} Process finished but reference still exists. Clearing manually."
                )
                self.server_process = None
            ...
            QMessageBox.critical(
                self, DIALOG_ERROR_TITLE, f"Error stopping server:\n{e}"
            )
        finally:
    ...
    def _handle_process_started(self):
        ...
        if (
            self.server_process
            and self.server_process.state() == QProcess.ProcessState.Running
        ):
            logging.info(
                f"{log_prefix} API server process confirmed running (PID: {self.server_process.processId()})."
            )
            ...
            logging.warning(
                f"{log_prefix} 'started' signal received, but process state is not Running ({self.server_process.state() if self.server_process else 'None'}). Likely failed to start."
            )
    ...
    def _handle_process_finished(self, exitCode: int, exitStatus: QProcess.ExitStatus):
        ...
        logging.warning(
            f"{log_prefix} API server process finished. Exit Code: {exitCode}, Exit Status: {status_string}"
        )

        # Avoid duplicate updates if _stop_server is managing the state
        if self.stopping_server:
            logging.debug(
                f"{log_prefix} Server finished during planned stop. Stop function will handle final UI."
            )
        ...
        if exitStatus == QProcess.ExitStatus.CrashExit or exitCode != 0:
            logging.error(
                f"{log_prefix} Server process stopped unexpectedly or crashed."
            )
            QMessageBox.warning(
                self,
                DIALOG_WARNING_TITLE,
                f"The API server process stopped unexpectedly.\nExit Code: {exitCode}\nStatus: {status_string}\nCheck server logs.",
            )
        ...
        if self.server_process:
            try:
                self.server_process.disconnect()  # Disconnect signals from this process object
            except Exception:
                pass
    ...
    def _handle_process_error(self, error: QProcess.ProcessError):
        ...
        error_string = (
            self.server_process.errorString()
            if self.server_process
            else "Unknown QProcess error"
        )
        logging.error(
            f"{log_prefix} QProcess error occurred: {error.name}. Details: {error_string}"
        )
        ...
        if error == QProcess.ProcessError.FailedToStart:
            ...
            QMessageBox.critical(
                self,
                DIALOG_ERROR_TITLE,
                f"API server process crashed during startup or execution.\nDetails: {error_string}",
            )
        else:
            ...
            QMessageBox.critical(
                self,
                DIALOG_ERROR_TITLE,
                f"An error occurred with the API server process:\n{error_string}",
            )
    ...
    def _handle_process_output(self):
        ...
        try:
            ...
            try:
                output_str = bytes(output_bytes).decode(
                    sys.stdout.encoding or "utf-8", errors="replace"
                )
            except Exception:
                output_str = bytes(output_bytes).decode("utf-8", errors="replace")
    ...
    def _update_ui_state(self):
        ...
        is_intermediate_state = self.status_label.text() in [
            API_STATUS_STARTING,
            API_STATUS_STOPPING,
        ]
