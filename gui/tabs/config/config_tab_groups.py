...
def _build_llm_data_group(config_tab):
    ...
    ).setMinimumWidth(250).setToolTip(
    ...
    # Tooltips for API key are handled inside _add_openai_api_key_setting
...
def _build_rebuild_settings_group(config_tab):
    ...
    ).setMaximumWidth(100).setToolTip(
...
def _build_api_group(config_tab):
    ...
    _wrap_checkbox(config_tab, "api.auto_start", "Auto Start API", layout).setToolTip(
...
def _build_advanced_group(config_tab):
    ...
    # Tooltip for the label is less critical as the slider has a detailed one.

    slider_layout.addWidget(slider)
    slider_layout.addWidget(slider_label)
    layout.addRow(QLabel("Hybrid Weight:"), slider_layout)

    _wrap_checkbox(config_tab, "cache_enabled", "Enable RAG Cache", layout).setToolTip(
...
def _build_qdrant_group(config_tab):
    ...
    add_config_setting(
        ...
        widget_type=QLineEdit,
    ).setMinimumWidth(200).setToolTip(
    ...
    add_config_setting(
        ...
        default_value=6333,
        widget_type=QSpinBox,
    ).setMaximumWidth(100).setRange(1024, 65535).setToolTip(
    ...
    add_config_setting(
        ...
        default_value="",
        widget_type=QLineEdit,
    ).setEchoMode(QLineEdit.EchoMode.Password).setToolTip(
...
def _build_logging_group(config_tab):
    ...
    add_config_setting_with_browse(
        ...
        directory=True,
    ).setToolTip("Directory where application log files will be stored.")
...
def build_chat_settings_group(tab):
    ...
    add_config_setting(
        ...
        widget_type=QLineEdit,
    ).setToolTip("The name displayed for the AI assistant in the chat interface.")
    ...
    def toggle_system_prompt():
        ...
        system_prompt_input_widget.setVisible(expanded)  # Use the correct variable name
