...
MSG_LOG_OPEN_ERROR = "Could not open log file location.\nPath: {logpath}\nError: {error}"
DIALOG_INFO_TITLE = ""
...
class StatusTab(QWidget):
    ...
    def __init__(self, config: MainConfig, project_root: Path, parent=None): # <<< ADD project_root
        ...
        if not pydantic_available:
            ...
            layout = QVBoxLayout(self); layout.addWidget(QLabel("Status Tab Disabled: Config system failed."))
        ...
        if not isinstance(config, MainConfig):
            ...
            layout = QVBoxLayout(self); layout.addWidget(QLabel("Status Tab Disabled: Invalid Configuration."))
            self._disable_init_on_error()
            return
        # Store project_root, even if not used directly by this tab currently.
        if not isinstance(project_root, Path):
             logging.warning(f"{log_prefix} Invalid project_root received ({type(project_root)}).")
        ...
        if self.log_path: # Start timer only if log path is valid
             ...
             logging.error(f"{log_prefix} Cannot start log timer or load logs: Log path not resolved.")
             if hasattr(self, 'log_output') and self.log_output:
                  self.log_output.setPlainText("ERROR: Log path configuration is missing or invalid. Check Configuration tab.")
    ...
    def _update_settings_from_config(self):
        ...
        if self.config is None:
             logging.error(f"{log_prefix} Config object is None. Cannot update settings.")
        ...
        if config_log_path and isinstance(config_log_path, Path):
            ...
            logging.error(f"{log_prefix} log_path in config is missing or invalid ({config_log_path}). Cannot display logs.")
        ...
        try:
            ...
            if not isinstance(self.log_refresh_ms, int) or self.log_refresh_ms < 500:
                logging.warning(f"{log_prefix} Invalid gui_log_refresh_ms ({self.log_refresh_ms}), using default {DEFAULT_LOG_REFRESH_MS}.")
                self.log_refresh_ms = DEFAULT_LOG_REFRESH_MS
            if not isinstance(self.log_lines_to_show, int) or self.log_lines_to_show < 10:
                logging.warning(f"{log_prefix} Invalid gui_log_lines ({self.log_lines_to_show}), using default {DEFAULT_LOG_LINES}.")
                self.log_lines_to_show = DEFAULT_LOG_LINES

            logging.debug(f"{log_prefix} GUI settings updated: Refresh={self.log_refresh_ms}ms, Lines={self.log_lines_to_show}")

        except AttributeError as e:
             logging.error(f"{log_prefix} Error accessing GUI settings from config: {e}. Using defaults.")
             self.log_refresh_ms = DEFAULT_LOG_REFRESH_MS
             self.log_lines_to_show = DEFAULT_LOG_LINES
    ...
    def init_ui(self):
        ...
        self.refresh_log_button.setToolTip("Manually reload the log display from the file.")
        ...
        self.clear_log_button.setToolTip(f"Permanently clear the contents of the log file ({self.log_path or 'N/A'}).")
        self.clear_log_button.clicked.connect(self.clear_logs)
        self.clear_log_button.setEnabled(bool(self.log_path)) # Enable only if path valid

        self.open_log_button = QPushButton(STATUS_OPEN_LOG_BUTTON) # New button
        self.open_log_button.setToolTip(f"Open the directory containing the log file ({self.log_path.parent if self.log_path else 'N/A'}).")
        self.open_log_button.clicked.connect(self.open_log_location)
        self.open_log_button.setEnabled(bool(self.log_path)) # Enable only if path valid

        self.auto_refresh_checkbox = QCheckBox(STATUS_LOG_AUTO_REFRESH_CHECKBOX)
        self.auto_refresh_checkbox.setToolTip("Automatically refresh the log display below every few seconds and scroll to the end.")
        ...
        self.log_output.setLineWrapMode(QTextEdit.LineWrapMode.NoWrap) # No wrapping for logs
        self.log_output.setFontFamily("Consolas, Courier New, monospace") # Use monospace font
        self.log_output.setStyleSheet("background-color: #f0f0f0;") # Light gray background
    ...
    def start_log_timer(self):
        ...
        try:
            ...
            if interval < 500: interval = 500 # Enforce minimum interval
            self.timer.setInterval(interval)
        except Exception as e:
             logging.error(f"{log_prefix} Invalid refresh interval {self.log_refresh_ms}, using default. Error: {e}")
        ...
        if self.auto_refresh_checkbox and self.auto_refresh_checkbox.isChecked():
             self.timer.start()
             logging.info(f"{log_prefix} Log refresh timer started (Interval: {self.timer.interval()}ms).")
        else:
             logging.info(f"{log_prefix} Log refresh timer initialized but not started (auto-refresh unchecked).")
    ...
    def _handle_auto_refresh_toggle(self, state):
        ...
        if not self.timer:
             ...
             if state == Qt.CheckState.Checked.value: self.start_log_timer()
             return
        ...
        if state == Qt.CheckState.Checked.value:
            if not self.timer.isActive():
                ...
                self.timer.start()
                logging.info(f"{log_prefix} Log auto-refresh timer started (Interval: {self.timer.interval()}ms).")
    ...
    def load_logs(self, is_manual_refresh=False):
        ...
        if not self.log_path or not isinstance(self.log_path, Path):
             ...
             if self.timer and self.timer.isActive(): self.timer.stop() # Stop timer if path invalid
             return

        # If called by timer, check if auto-refresh is still enabled
        if not is_manual_refresh and self.auto_refresh_checkbox and not self.auto_refresh_checkbox.isChecked():
        ...
        try:
            ...
            if not log_file.is_file():
                 ...
                 if is_manual_refresh: logging.warning(f"Log file not found at: {self.log_path}")
                 return
            ...
            try:
                ...
                last_lines_text = "".join(tail_lines)
                 ...
                 raise IOError(f"OS error reading file content: {read_err}") from read_err
            except Exception as read_err:
                 raise IOError(f"Unexpected error reading file content: {read_err}") from read_err
            ...
            logging.error(f"{log_prefix} Failed to load logs from '{self.log_path}': {e}", exc_info=True)
            self.log_output.setPlainText(error_message)
        except Exception as e:
            # Catch other potential errors
            error_message = MSG_LOG_LOAD_ERROR.format(logpath=self.log_path, error=e)
            logging.exception(f"{log_prefix} Unexpected error loading logs from '{self.log_path}'.")
            self.log_output.setPlainText(error_message)
    ...
    def clear_logs(self):
        ...
        if not self.log_path or not isinstance(self.log_path, Path):
            logging.error(f"{log_prefix} Cannot clear logs: Log path is invalid or not set.")
            QMessageBox.critical(self, DIALOG_ERROR_TITLE, "Cannot clear log: Log path not configured correctly.")
            return
        ...
        try:
            ...
            if not log_dir.exists():
                logging.info(f"{log_prefix} Log directory not found, attempting to create: {log_dir}")
            ...
            if self.log_output: self.log_output.setPlainText("")
            logging.info(f"{log_prefix} Log file cleared successfully: {log_file}")
            QMessageBox.information(self, DIALOG_INFO_TITLE, f"Log file content cleared:\n{log_file}")

        except (OSError, IOError) as e:
            error_message = MSG_LOG_CLEAR_ERROR.format(logpath=log_file, error=e)
            logging.error(f"{log_prefix} Failed to clear log file '{log_file}': {e}", exc_info=True)
            QMessageBox.critical(self, DIALOG_ERROR_CLEAR_LOGS, error_message)
            if self.log_output: self.log_output.setPlainText(error_message) # Show error in display
        except Exception as e:
             error_message = MSG_LOG_CLEAR_ERROR.format(logpath=log_file, error=e)
             logging.exception(f"{log_prefix} Unexpected error clearing log file '{log_file}'.")
             QMessageBox.critical(self, DIALOG_ERROR_CLEAR_LOGS, error_message)
             if self.log_output: self.log_output.setPlainText(error_message)
    ...
    def open_log_location(self):
        ...
        if not self.log_path or not isinstance(self.log_path, Path):
            ...
            QMessageBox.warning(self, DIALOG_WARNING_TITLE, "Log path is not configured correctly.")
            return
        ...
        try:
            ...
            logging.error(f"{log_prefix} Failed to open log location '{log_dir}': {e}", exc_info=True)
            QMessageBox.critical(self, DIALOG_ERROR_TITLE, error_message)


    # --- Configuration Update Slot ---
    def update_config(self, new_config: MainConfig):
        """Slot called by main_window when config changes externally."""
        logging.info(f"--- StatusTab.update_config called. New Config ID: {id(new_config)} ---")
        ...
        if self.timer and self.timer.interval() != self.log_refresh_ms:
             ...
             logging.info(f"Log refresh timer interval updated to: {self.log_refresh_ms} ms")
        ...
        if hasattr(self, 'clear_log_button'): self.clear_log_button.setEnabled(can_open_clear)
        if hasattr(self, 'open_log_button'): self.open_log_button.setEnabled(can_open_clear)
        if hasattr(self, 'clear_log_button'): self.clear_log_button.setToolTip(f"Permanently clear the contents of the log file ({self.log_path or 'N/A'}).")
        if hasattr(self, 'open_log_button'): self.open_log_button.setToolTip(f"Open the directory containing the log file ({self.log_path.parent if self.log_path else 'N/A'}).")
        ...
        if self.log_path:
             ...
             logging.error("Cannot refresh logs: Log path became invalid after config update.")
             if hasattr(self, 'log_output') and self.log_output:
                  self.log_output.setPlainText("ERROR: Log path configuration is missing or invalid after config update.")