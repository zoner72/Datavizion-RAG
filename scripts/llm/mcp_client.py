...
try:
    ...
    logging.critical(
        "MCP library not found or could not be imported. MCP functionality disabled."
    )
    mcp_available = False
...
async def query_mcp(query_str: str) -> str:
    ...
    try:
        ...
        if not server_script_path.is_file():
            ...
            logging.warning(
                f"Could not find server script at {server_script_path}, using relative path '{server_script_path_arg}'"
            )
        else:
    ...
    server_params = StdioServerParameters(
        command=sys.executable, args=[server_script_path_arg]
    )
    logging.info(
        f"Starting MCP server subprocess: {sys.executable} {server_script_path_arg}"
    )
    ...
    try:
        ...
        async with stdio_client(server_params) as (read, write):
            ...
            async with ClientSession(read, write) as session:
                ...
                logging.debug(
                    "MCP Client: Session initialized. Calling tool 'rag_query'..."
                )

                try:
                    result = await session.call_tool(
                        "rag_query", arguments={"query": query_str}
                    )
                    logging.debug("MCP Client: Tool call successful.")
                    return (
                        str(result)
                        if result is not None
                        else "[MCP Info] Tool returned None"
                    )
                ...
                # Add more specific MCP exception handling if needed
                    ...
                    logging.error(
                        f"MCP Error during tool call: {tool_e}", exc_info=True
                    )
        ...
        logging.error(
            f"Error setting up MCP stdio client/session: {client_e}", exc_info=True
        )
