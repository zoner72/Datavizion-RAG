...
try:
    ...
    if str(project_root) not in sys.path:
        ...
        print(
            f"[MCP Server] Added project root to sys.path: {project_root}",
            file=sys.stderr,
        )
except Exception as path_e:
    print(
        f"[MCP Server] Warning: Could not easily add project root to sys.path: {path_e}",
        file=sys.stderr,
    )
# --------------------------------------------------
...
try:
    ...
    print(
        f"[MCP Server] CRITICAL: Failed to import core components: {e}", file=sys.stderr
    )
...
try:
    ...
    print(
        "[MCP Server] CRITICAL: MCP library not found or could not be imported.",
        file=sys.stderr,
    )
    mcp_available = False
    ...
    class FastMCP:
        ...
        def run(self):
            """Dummy run method."""
            print(
                "[MCP Server] MCP dummy run (MCP library not installed).",
                file=sys.stderr,
            )
...
def load_config_and_init():
    ...
    if not pydantic_available or not core_components_available:
        logger.critical(
            "Cannot initialize: Pydantic or core components failed to import."
        )
    ...
    try:
        ...
        if not config_path_str:
            raise ValueError("'KNOWLEDGE_LLM_CONFIG_PATH' env var not set.")
        logger.info(f"Loading configuration from path: {config_path_str}")
        config = load_config_from_path(config_path_str)
        if config is None or not Path(config_path_str).exists():
            raise ValueError(f"Failed to load valid config from {config_path_str}")
        ...
        if not load_provider_modules(config):
            logger.warning("LLM provider modules check failed.")
        index_model_name = config.embedding_model_index
        query_model_name = config.embedding_model_query
        if not index_model_name:
            raise ValueError("Index embedding model not specified.")
        ...
        if not index_manager.check_connection():
            logger.warning("Initial Qdrant check failed.")
        else:
            logger.info("Qdrant Manager OK.")
...
if not mcp_available:
    logger.critical("MCP library missing. Exiting.")
    sys.exit(1)
if not load_config_and_init():
    logger.critical("Server init failed. Exiting.")
    sys.exit(1)
# --------------------------
    ...
    async def rag_query(query: str, context: Context) -> str:
        ...
        try:
            ...
            (
                ...
                retrieved_docs,
            ) = await memory_context_manager.get_context_for_query(
            ...
            answer = generate_answer(
                query=query, retrieved_docs=retrieved_docs, config=config
            )
...
if __name__ == "__main__":
    if mcp and not initialization_failed:
        ...
        try:
            ...
            logger.critical(f"MCP server run failed: {run_e}", exc_info=True)
            sys.exit(1)
    elif initialization_failed:
        logger.critical("Exiting: MCP server initialization failed.")
        sys.exit(1)
    else:
        logger.critical("Exiting: MCP library not available.")
        sys.exit(1)
