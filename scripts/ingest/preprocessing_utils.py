...
def worker_print(pid, *args):
    ...
    try:
        ...
        try: print(f"Preprocessing Worker Print Error: {e}")
        except: pass
# ------------------------------------
...
def remove_boilerplate(text: str) -> str:
    ...
    if not text: return ""
    lines = text.splitlines(); cleaned_lines = []; min_words_threshold = 4
    for line in lines:
        stripped_line = line.strip()
        if not stripped_line: continue
        line_lower = stripped_line.lower(); words = stripped_line.split()
        contains_keyword = any(keyword in line_lower for keyword in BOILERPLATE_KEYWORDS)
        if contains_keyword: continue # worker_print(...) # Verbose
        if len(words) < min_words_threshold and not (stripped_line.endswith(':') or stripped_line.isupper()): continue # worker_print(...) # Verbose
        if stripped_line.count('|') >= 3: continue # worker_print(...) # Verbose
...
def extract_enhanced_metadata(filepath: str, text_content: str, fields_to_extract: list) -> dict:
    ...
    if "product_name" in fields_to_extract:
        if "digibox" in filename_lower or "digibox" in first_lines: metadata["product_name"] = "Digibox"
        elif "quantumx" in filename_lower or "quantumx" in first_lines: metadata["product_name"] = "QuantumX"
    if "model_number" in fields_to_extract:
        match = re.search(r'(model|part number|p/n)\s*[:\-]?\s*([A-Za-z0-9\-/\s]+)', first_lines, re.IGNORECASE)
        if match: metadata["model_number"] = re.split(r'\s{2,}', match.group(2).strip())[0]
    if "section_title" in fields_to_extract:
         potential_titles = [s.strip(':').strip() for line in text_content.splitlines()[:20] if (s := line.strip()) and (s.isupper() or s.endswith(':')) and 3 < len(s.split()) < 10]
         if potential_titles: metadata["section_title"] = potential_titles[0]
    if "document_type" in fields_to_extract:
         if metadata.get("inferred_doc_type") and metadata["inferred_doc_type"] != "unknown": metadata["document_type"] = metadata["inferred_doc_type"]
         else: # Fallback checks...
            if "manual" in filename_lower: metadata["document_type"] = "manual"
            # ... other type checks ...
    # worker_print(pid, f"Enhanced metadata extracted: { {k:v for k,v in metadata.items() if k != 'filename'} }") # Verbose
    return metadata

def extract_metadata_with_llm(text_content: str, fields_to_extract: list, llm_model_name: str) -> dict:
