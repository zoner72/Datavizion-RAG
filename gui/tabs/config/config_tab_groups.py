...
def _build_rebuild_settings_group(config_tab):
    ...
    add_config_setting(
        ...
        widget_type=QLineEdit,
    )  # Corrected default to match JSON
    add_config_setting(
        config_tab,
        layout,
        "Query Model:",
    ...
    ).setMaximumWidth(100)  # Corrected default
    add_config_setting_with_browse(
        config_tab,
        layout,
        "Embedding Cache Dir:",
...
def _build_advanced_group(config_tab):
    ...
    config_tab.ui_widgets["weight_display_label"] = (
        slider_label  # Store label reference
    )

    slider_layout.addWidget(slider)
    slider_layout.addWidget(slider_label)
    layout.addRow(
        QLabel("Hybrid Weight:"), slider_layout
    )  # Add as a single row in QFormLayout
...
def _build_qdrant_group(config_tab):
    ...
    if (
        hasattr(config_tab, "settings_widgets")
        and "qdrant.quantization_enabled" in config_tab.settings_widgets
    ):
        quant_enabled_checkbox = config_tab.settings_widgets[
            "qdrant.quantization_enabled"
        ]
...
def build_chat_settings_group(tab):
    ...
    assistant_name_input = add_config_setting(
        tab,
        layout,
        "Assistant Name:",
        "assistant_name",
    ...
    tab.prompt_template_toggle_button = (
        collapsible_button  # Store the button reference in ConfigTab instance
    )

    prompt_template_input = add_config_setting(
        tab,
        layout,
        "Prompt Template:",
        "prompt_template",
    ...
    )  # Use add_config_setting for QTextEdit
    ...
    def toggle_prompt_template():
        ...
        collapsible_button.setText(
            "🔽 Prompt Template (Click to Collapse)"
            if expanded
            else "▶️ Prompt Template (Click to Expand)"
        )
